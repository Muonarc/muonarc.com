...
**Status:** HITL paste-ready. AI-drafted. Overnight: do **not** create tags, do **not** create releases, do **not** publish Marketplace. Lands on `cursor/notes-surface-a3e9` only. Not main. Not merged. Do **not** refresh `drafts/notes-first-morning-hitl-2026-09-02.md`.

**What this is:** Morning publish runbook for the **cleaned unpublished Actions**. Queue: nws → usgs → openfda → openfema-declarations → openfema-pa-action → **cisa-kev-action**. Free utilities / Continuous integration. **No paid SKU. No Ko-fi. No hospital MRF-change extract. No $40 CTAs. No OpenFEMA shop SKU.**
...
1. Open the repo URL.
2. Confirm `main` is still the cleaned HEAD named here (or a later commit that keeps the CTA strip / CI smoke). If `main` rewound to a pre-strip or scaffold-only SHA, **stop that repo** and ping Atlas.
3. **Releases → Draft a new release.**
4. Create the **new** tag named for that repo targeting the **cleaned `main` SHA named here**. Repos 1–5: **`v0.1.1`**. Repo 6 (cisa-kev-action): **`v0.1.0` from `54ff5dd2`** (CI smoke included; **not** scaffold-only `4c6e3ec7`). **Do not** move or retag existing `v0.1.0` on repos 1–5 (those still peel pre-strip paid CTAs).
...
- **Tag `v0.1.0` from cleaned main `54ff5dd2`** (CI smoke included). First tag — there is no prior tag to retag. Do **not** tag scaffold-only `4c6e3ec7`. Do **not** tag listing-only `2d804188`.
- Listing HITL: `2d804188` — `drafts/MARKETPLACE-LISTING.md` (morning-must: tag **54ff5dd2** before publish)
...
- Tag cisa-kev-action from `4c6e3ec7`
- Refresh `drafts/notes-first-morning-hitl-2026-09-02.md`
