# HITL morning card — Friday COT Pack borrowed shares (2026-09-13)

> **Scoreboard:** first paid dollar via **Friday COT Pack** only. Freeze USA / NCUA / Marketplace / nist-nvd / cold email / killed rails until COT has taken money.

**Status:** HITL / Atlas+Benjamin morning card for **2026-09-13**. **NEW file** — does **not** refresh frozen Sep-11 @ notes SHA `9ce64e65` (or older Sep-10/9/8/7 cards). **DO-NOT-POST** until Atlas / Benjamin says. Rogue does not post, email, mutate Pages, or open frozen rails.

```
LIVE NOTE (http until Pages Enforce HTTPS): http://muonarc.com/notes/friday-cot-pack.html
CHECKOUT ($18 Friday COT Pack): https://buy.stripe.com/eVqfZi16bbHibem4l95AQ00
WEEK: as-of Tuesday 2026-09-08 / CFTC released Friday 2026-09-11
```

**AI disclosure (Rogue):** An AI drafted these paste pointers and this card. Benjamin / Atlas reviews and posts (if any).

---

## If you only have 90 seconds

1. **Pages Enforce HTTPS** — fix SSL CN=`*.github.io` vs `muonarc.com`. Until fixed, keep every share URL as `http://muonarc.com/notes/friday-cot-pack.html` (do not paste https Note URLs).
2. **Post Ready paste only** (below) — then **stop**.
3. **Watch Gmail payment-detect** — `from:stripe.com (payment OR receipt OR "Payment Link") newer_than:7d` (ignore Render / Grok / device). If COT / Friday COT Pack / $18 / muonarc paid → open fulfill `drafts/notes-first-cot-fulfillment-email-2026-09-01.md` @ notes **`d798df5c`**. **DO-NOT-SEND** until Atlas confirms.

---

## Ready paste (one-shot approve/post)

**Recommended post:** this Ready paste only. Do not invent a second venue until Atlas greens backlog.

### Paste (do not post until Atlas / Benjamin says)

```
Same CFTC week (positions as-of 2026-09-08 / released Sep 11): yen specs flipped net long +10,796 after a +103,023 week (largest swing in the pack). Soybean specs +273,424 — new 52-week high. Nat-gas specs −219,767 — 52-week short extreme (sold another −10,856). Corn specs still at a 52-week high (+542,999).

Packaged for a fixed 12-name Legacy Futures-Only set (CL, NG, GC, SI, ZC, ZS, ZW, ES, NQ, 6E, 6J, BTC): one-pager + CSV with net spec / commercial / non-reportable, 1w + 4w change, and 52w percentile.

Note: http://muonarc.com/notes/friday-cot-pack.html
$18 one-time: https://buy.stripe.com/eVqfZi16bbHibem4l95AQ00

Built by Rogue (AI); Benjamin reviews before sale. Not a CFTC product. Independently check any number you rely on.
```

---

## Prior morning cards (DO-NOT-POST / frozen — leave untouched)

| Card | Path | Notes SHA |
| --- | --- | --- |
| Sep-11 (Armchair Ready) | `drafts/notes-first-cot-borrowed-shares-morning-hitl-2026-09-11.md` | `9ce64e65` |
| Sep-10 (Bloodstone Ready) | `drafts/notes-first-cot-borrowed-shares-morning-hitl-2026-09-10.md` | `afb59c5e` |
| Sep-9 (Ag Optimus Ready) | `drafts/notes-first-cot-borrowed-shares-morning-hitl-2026-09-09.md` | `561ba5b2` |
| Sep-8 | `drafts/notes-first-cot-borrowed-shares-morning-hitl-2026-09-08.md` | `c5398664` |
| Sep-7 | `drafts/notes-first-cot-borrowed-shares-morning-hitl-2026-09-07.md` | `92517f10` |

---

## First paid dollar — fulfill in 60s

| Pin | Exact |
| --- | --- |
| Live Note | `http://muonarc.com/notes/friday-cot-pack.html` |
| Stripe checkout | `https://buy.stripe.com/eVqfZi16bbHibem4l95AQ00` ($18) |
| Buyer one-pager | `drafts/notes-first-cot-pack-2026-09-11.md` @ main **`e6ca80fd`** — blob **`d764c6a7`** (10871B) |
| Buyer CSV | `drafts/cot-pack-2026-09-11.csv` @ main **`e6ca80fd`** — blob **`d9c4f82f`** (3081B) |
| Fulfill email template | `drafts/notes-first-cot-fulfillment-email-2026-09-01.md` @ notes **`d798df5c`** — **do-not-send** until Atlas confirms a paid checkout |

**Payment detect:** Gmail `from:stripe.com (payment OR receipt OR "Payment Link") newer_than:7d` — ignore Render / Grok / device. COT/$18/muonarc hit → YES Atlas with thread id + open fulfill `d798df5c`. **DO-NOT-SEND** yourself.

**60s path:** Atlas confirms paid checkout → open template @ notes **`d798df5c`** → attach one-pager blob `d764c6a7` + CSV blob `d9c4f82f` → HITL send.

---

## Pages Enforce HTTPS (step 0 detail)

GitHub Pages currently serves a `*.github.io` cert for `muonarc.com`. Until **Enforce HTTPS** is on and the cert validates, keep share / paste URLs as:

```
http://muonarc.com/notes/friday-cot-pack.html
```

Rogue does **not** mutate Pages settings.

---

## Freeze / Out

- Post without Atlas/Benjamin go
- Refresh frozen Sep-11 @ `9ce64e65` (or older morning cards)
- Paste `https://muonarc.com/...` before Pages cert is fixed
- Cold email / Alera / business outbound
- USA Spending / NCUA+Title12 / Marketplace / nist-nvd / Pages mutation by Rogue
- Create Stripe / second SKU / invent checkout URL
- Fulfill send before paid checkout
- Extra borrowed-share drafts after this card lands (FREEZE packaging for the overnight window)

**STOP holds.** HOLD COT-only. **DO-NOT-POST** until Atlas / Benjamin.

AI-drafted by Rogue. Benjamin / Atlas owns Pages HTTPS HITL and any human post.
